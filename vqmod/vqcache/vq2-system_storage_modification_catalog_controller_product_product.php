...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$this->document->addLink($this->url->link('product/product', 'product_id=' . $this->request->get['product_id'] . ( $this->config->get( 'smp_c_short_product_urls' ) ? '&path=0' : '' ) ), 'canonical');
            ...
            $data['show_buy'] = $product_info['buybtnsec'];
			...
			if ($product_info['image']) {
				$data['popup'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get($this->config->get('config_theme') . '_image_popup_width'), $this->config->get($this->config->get('config_theme') . '_image_popup_height'));
			...
			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $this->config->get($this->config->get('config_theme') . '_image_thumb_width'), $this->config->get($this->config->get('config_theme') . '_image_thumb_height'));
			} else {
				$data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					...
					'thumb' => $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get($this->config->get('config_theme') . '_image_additional_width'), $this->config->get($this->config->get('config_theme') . '_image_additional_height'))
				);
			...
			foreach ($results as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
				} else {
...
//            my related
            $data['products_true'] = array();
           
            $results_true = $this->model_catalog_product->getProductTrueRelated($this->request->get['product_id']);

			foreach ($results_true as $result) {
				if ($result['image']) {
					$image = $this->model_tool_image->resize($result['image'].(isset($result['product_id'])?'::'.$result['product_id']:''), 150, 150);
				} else {
					$image = $this->model_tool_image->resize('placeholder.png', $this->config->get($this->config->get('config_theme') . '_image_related_width'), $this->config->get($this->config->get('config_theme') . '_image_related_height'));
				}
			...
			$data['footer'] = $this->load->controller('common/footer');
				...
				if( NULL != ( $tmp = (array) $this->config->get( 'smp_facebook_open_graph' ) ) && ! empty( $tmp[$this->config->get('config_store_id')] ) ) {
					...
					$this->document
						...
						->addMeta( 'og:image', $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $config_image_popup_width, $config_image_popup_height), 'property' )
				...
				if( NULL != ( $tmp = (array) $this->config->get( 'smp_twitter_cart' ) ) && ! empty( $tmp[$this->config->get('config_store_id')] ) ) {
					...
					$this->document
						...
						->addMeta( 'twitter:image:src', $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $config_image_popup_width, $config_image_popup_height) )
				...
				if( NULL != ( $tmp = (array) $this->config->get( 'smp_googleplus_metadata' ) ) && ! empty( $tmp[$this->config->get('config_store_id')] ) ) {		
					...
					$this->document
						...
						->addMeta( 'image', $this->model_tool_image->resize($product_info['image'].(isset($product_info['product_id'])?'::'.$product_info['product_id']:''), $config_image_popup_width, $config_image_popup_height), 'itemprop' ) 
