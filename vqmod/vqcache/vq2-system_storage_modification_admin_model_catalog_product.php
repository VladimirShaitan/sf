...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', buybtnsec = '" . $this->db->escape($value['buybtnsec']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
		...
		if (isset($data['product_related'])) {
			foreach ($data['product_related'] as $related_id) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$product_id . "', related_id = '" . (int)$related_id . "', smp_ag = '" . ( isset( $productBy ) && in_array( $related_id, $productBy ) ? '1' : '0' ) . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_related WHERE product_id = '" . (int)$related_id . "' AND related_id = '" . (int)$product_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "', smp_ag = '" . ( isset( $productBy ) && in_array( $related_id, $productBy ) ? '1' : '0' ) . "'");
			}
		}
        
        /*My related*/
        
       if (isset($data['product_true_related'])) {
			foreach ($data['product_true_related'] as $related_id) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_true_related WHERE product_id = '" . (int)$product_id . "' AND related_id = '" . (int)$related_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_true_related SET product_id = '" . (int)$product_id . "', related_id = '" . (int)$related_id . "', smp_ag = '" . ( isset( $productBy ) && in_array( $related_id, $productBy ) ? '1' : '0' ) . "'");
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_true_related WHERE product_id = '" . (int)$related_id . "' AND related_id = '" . (int)$product_id . "'");
				$this->db->query("INSERT INTO " . DB_PREFIX . "product_true_related SET product_id = '" . (int)$related_id . "', related_id = '" . (int)$product_id . "', smp_ag = '" . ( isset( $productBy ) && in_array( $related_id, $productBy ) ? '1' : '0' ) . "'");
			}
		}
	...
	public function editProduct($product_id, $data) {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_true_related WHERE product_id = '" . (int)$product_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_true_related WHERE related_id = '" . (int)$product_id . "'");
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
            ...
            /*My related*/
			$data['product_true_related'] = $this->getProductTrueRelated($product_id);
            /*My related*/
...
//        My related
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, " . ( $this->config->get('smp_sug_is_install') ? "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "' AND (smp_language_id IS NULL OR smp_language_id = " . $this->config->get('config_language_id') . ") ORDER BY smp_language_id DESC LIMIT 1 ) AS keyword" : "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "' LIMIT 1 ) AS keyword" ) . " FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
...
//    My related
    	public function getProductTrueRelated($product_id) {
		$product_true_related_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_true_related WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_true_related_data[] = $result['related_id'];
		}

		return $product_true_related_data;
	}
