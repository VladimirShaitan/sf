	...
	public function addBanner($data) {
		...
		if (isset($data['banner_image'])) {
			foreach ($data['banner_image'] as $banner_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "banner_image SET banner_id = '" . (int)$banner_id . "', link = '" .  $this->db->escape($banner_image['link']) . "', choose = '" .  $this->db->escape($banner_image['choose']) . "', video_link = '" .  $this->db->escape($banner_image['video_link']) . "', videoLogo = '" .  $this->db->escape($banner_image['videoLogo']) . "', videoDesc = '" .  $this->db->escape($banner_image['videoDesc']) . "', image = '" .  $this->db->escape($banner_image['image']) . "', sort_order = '0'");
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			$banner_image_data[] = array(
				...
				'choose'                   => $banner_image['choose'],
				'video_link'               => $banner_image['video_link'],
				'videoLogo'                => $banner_image['videoLogo'],
				'videoDesc'                => $banner_image['videoDesc'],
