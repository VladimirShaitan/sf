...
//    my related
		...
		$product_true_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_true_related ptr LEFT JOIN " . DB_PREFIX . "product p ON (ptr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE ptr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

		foreach ($query->rows as $result) {
			$product_true_data[$result['related_id']] = $this->getProduct($result['related_id']);
		}

		return $product_true_data;
	}
