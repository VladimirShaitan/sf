...
class ControllerModulePopupCart extends Controller {
	...
	public function index() {
		...
		$this->load->language('module/popup_cart');
		...
		$data['heading_title_empty'] = $this->language->get('heading_title_empty');
    ...
    if (isset($this->request->request['add'])) {
		...
		$data['checkout_link'] = $this->url->link('checkout/cart');
		$data['heading_title'] = sprintf($this->language->get('heading_title'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0));

    $this->response->setOutput($this->load->view('module/popup_cart.tpl', $data));
	}
